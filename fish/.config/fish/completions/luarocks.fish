...
complete -c luarocks -l pin -d 'Create a luarocks.lock file listing the exact versions of each dependency found for this rock (recursively), and store it in the rock\'s directory'
...
complete -c luarocks -n '__fish_luarocks_seen_command build' -l only-deps -d 'Install only the dependencies of the rock'
...
complete -c luarocks -n '__fish_luarocks_seen_command install' -l only-deps -d 'Install only the dependencies of the rock'
